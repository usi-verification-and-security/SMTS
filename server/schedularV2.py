...
class Solver(net.Socket):
    ...
    def ask_partitions(self, n, node: framework.AndNode = None):
        ...
        if isinstance(self.node, framework.SMT):
            ...
            print("             Partition emited from ",estimate_partition_time)
        if not node:
    ...
    def read(self):
        ...
        #     print("                 masoud4 ",header['node'],self.node.root.name,header['name'],str(self.node.path()))
        #     return {}, b''
        ...
        del header['name']
        # del header['node']
        if header['report'] == 'partitions':
            for n in self.node.root.all():
                if header['node'][1:len(header['node'])-1] == ', '.join(map(str, n.path())):
                    print("   that's it . . ..........", n)


        if header['report'] == 'partitions' and self.or_waiting:
            # print("             hereeeeeee . . ..........",self)
            global estimate_partition_time
    ...
    def entrust(self):
        ...
        if config.partition_timeout:
            ...
            def un_attempted_active_leaves():
                return (node for node in nodes if (node.status == framework.SolveStatus.unknown and
                                                   isinstance(node, framework.AndNode) and node.assumed_timout) or
                        (len(node) == 0 and node.status == framework.SolveStatus.unknown and
            ...
            for node in nodes:
                if isinstance(node, framework.AndNode) and node.started is not None and not node.processed:
                    if node.status == framework.SolveStatus.unsat:
                    ...
                    elif not node.is_timeout and (node.started + config.node_timeout <= time.time()):
                        # print("     partition leave timout-> " , len(self.solvers(False)),totalN_partitions,node)
                        # node.processed = True
                        if node.partitioning:
                            ...
                            if node.assumed_timout:
                                ...
                                for solver in self.solvers(node):
                                    print('              node has been assumed_timout - done partition -> ',solver )
                                    self.idle_solvers.add(solver)
                                    # totalN_partitions -= 1
                            else:
                                for solver in self.solvers(node):
                                    print('              Timeout solver - has partition -> ',solver,config.node_timeout )
                                    self.idle_solvers.add(solver)
                                    totalN_partitions -= 1
                            ...
                            # if self.solvers(node) and len(self.idle_solvers) >= config.partition_policy[1]:
                            #     print('    Timeout solvers n-p ATO ',len(self.idle_solvers) ,solver)
                            node.assumed_timout = True
                            # if len(self.idle_solvers) == config.partition_policy[1]:
                            # config.node_timeout += estimate_partition_time+estimate_partition_time
            ...
            if partition_recieved():
                if not movable_solvers:
                    if config.portfolio_min:
                        # self.config.portfolio_min = math.ceil(self.total_solvers / (totalN_partitions + 1))
                        for _node in (node for node in all_current_active_nodes() if len(node) == self.level_children(len(node.path()))):
                            try:
                                print("     node ", node)
                                for child in _node:
                                    if len(child) == 0:
            ...
            if self.idle_solvers:
                ...
                # unattempted_notPartitioned = list(un_attempted_active_leaves())
                # unattempted_notPartitioned = sorted(unattempted_notPartitioned, key = lambda leave: leave)
            ...
            attempted_notPartitioned = sorted(attempted_notPartitioned, key = lambda leave: leave)

            for leaf in attempted_notPartitioned:
                # print("     partition leave -> " , self.total_solvers-1, totalN_partitions, leaf)
                if config.portfolio_min:
                    if self.total_solvers > totalN_partitions+config.partition_policy[1]:
                        # print("     partition leave -> " , len(self.solvers(False)),totalN_partitions,leaf)
                        # if len(movable_solvers) >= config.partition_policy[1]:
                        #     print("     movable_solvers to partition" , len(movable_solvers))
                        # movable_solvers.clear()
                        self.partition(leaf)
                else:
                    if self.total_solvers > totalN_partitions+config.partition_policy[1]:
                        self.partition(leaf)
