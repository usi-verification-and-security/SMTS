...
class ParallelizationServer(net.Server):
    ...
    def entrust(self):
        ...
        # print("\n idle at the first place -> ",len(idle_solvers))
        ...
        for node in nodes:
            ...
            #     # print('\n\n\n')
            #     for solver in self.solvers(node):
            #         # print('t_solver -> ',solver)
            #         if solver.started + config.node_timeout < time.time():
            #             for t_solver in self.solvers(node):
            #                 print('\n','t_solver -> ',t_solver)
            #                 idle_solvers.add(t_solver)
            #             break
                ...
                for node in selection():
                    ...
                    if not idle_solvers:
                        for _node in (node for node in internals() if
                            ...
                            # print("     self.config.portfolio_min->    ",self.config.portfolio_min,totalN_partitions+1, len(solvers))
                            if 0 < self.config.portfolio_max < len(solvers) or self.config.portfolio_min <= 1:
                                if self.config.portfolio_min < len(solvers) or self.config.portfolio_min <= 1:
                                    try:
                                        ...
                                        for solver in self.solvers(_node):
                                            if self.config.portfolio_min > 1:
                                                ...
                                                if self.config.node_timeout and solver.started + self.config.node_timeout > time.time():
                                                    continue
                                                print(" \n")
                                                for t_solver in self.solvers(_node):
                                                    idle_solvers.add(t_solver)

                                                    print("   add to idles -> ",solver, _node)
                                            raise StopIteration
                                    except StopIteration:
                                        break
