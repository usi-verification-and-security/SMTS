...
class Solver(net.Socket):
    ...
    def ask_partitions(self, n, node: framework.AndNode = None):
        ...
        self._db_log('OR', {constant.NODE: str(node.path()), constant.SOLVER: str(self.remote_address)})
        ...
        if header[constant.REPORT] == constant.PARTITIONS and self.or_waiting:
            ...
            try:
                ...
                for partition in payload.decode().split('\0'):
                    ...
                    self._db_log('AND', {constant.NODE: str(child.path()), 'smt': child.smt})
                ...
                try:
                    if config.spit_preference:
                        for sp in framework.SplitPreference.__members__.values():
                            ...
                            self.trees[header[constant.NAME] + sp.value] = instance
                    else:
                        instance = Instance(header[constant.NAME], payload.decode())
                        instance.timeout = config.solving_timeout
        ...
        if config.enableLog:
            ...
            if config.spit_preference:
                ...
                if message == 'unknown':
                    ...
                    res = 'Failed'
                    if header is not None and header['status_info'] == message or header['status_info'] == 'unknown':
                        res = 'Passed'
                    if header[constant.NODE] == '[]':
                        level_info = 'root'
                    else:
                        level_info = 'child'
