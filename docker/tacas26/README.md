...
The `quick` mode uses 5-min timeout and randomly selects 15 benchmarks from QF_LRA and 40 benchmarks from QF_LIA from a subset of benchmarks where one candidate per a group with similar runtimes was selected (`QF_L?A_files_uniq`: 576 and 144 benchmarks, resp.).
...
with `make-plots` command for the respective mode.
